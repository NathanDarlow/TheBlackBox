<template>
  <div>
    <v-container>
      <h1 class="my-10 text-center grey--text">Edit a Job</h1>
      <v-layout row wrap>
         <v-flex xs12>
           <div style="padding: 0.25em; margin: 0 auto; width: 70%;">
              <form action="" v-if="!contentStillLoading">
               <v-layout row wrap>
                  <v-flex xs6 pa-1>
                    ...
                    outlined
                    shaped
                    v-model="employersJob.jobTitle"></v-text-field>
                 </v-flex>
                 <v-flex xs6 pa-1>
                   <v-text-field 
                   label="Location"
                   outlined
                   shaped
                   v-model="employersJob.location"></v-text-field>
                 </v-flex>
                </v-layout>
                <v-layout row wrap>
                 <v-flex xs12>
              ...
              <v-textarea label="Job Description"
              outlined
              shaped
              v-model="employersJob.description"></v-textarea>
              
              <v-layout justify-center>
                <v-btn @click="updateJob">Update Job</v-btn>
              </v-layout>
         ...
         <v-flex xs6 v-if="updated">
            <div justify-center>
              <h2 style="text-align:center; margin: 5% auto;"><span style="color: green;">Job Updated!</span></h2>
               <router-link :to="{name:'view.employer.profile', params:{employerId: $store.state.employer.id}}" style="display: block; text-align:center; margin: 5% auto;">Back to Account</router-link>
            </div>
         </v-flex>
         <v-flex xs12>
