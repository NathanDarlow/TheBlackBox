<template>
  <div>
     <v-container>
        <h1 class="grey--text text-center my-10">View Applicants</h1>
        <v-divider class="mx-12"></v-divider>
       <v-layout row wrap v-if="noApplicants">
         <v-flex>
           <h2 class="grey--text text-center my-5">No Applicants at this time. Check back later.</h2>
         </v-flex>
       </v-layout>
       <v-layout row wrap v-if="!noApplicants">
            <v-flex xs12 v-for="applicant in applicants" :key="applicant.id" style="max-width: 1200px; margin: 10px auto;">
                  <h2 class="my-3"><span class="grey--text"><strong>{{ applicant.Jobseeker.firstName }} {{ applicant.Jobseeker.lastName }}</strong> - applying for the</span>&nbsp;<strong><span class="grey--text"> <span style="text-decoration: underl">{{applicant.Job.jobTitle}} </span></span></strong><span class="grey--text">position</span></h2>
              <v-card>
              <table>
                <thead>
                  <tr class="table-row">
                    <th></th>
                    <th class="title grey--text">Resume</th>
                    <th class="title grey--text">Cover Letter</th>
                    <th class="title grey--text">Email</th>
                    <th class="title grey--text">Address</th>
                    <th class="title grey--text">Phone</th>
                    <th class="title grey--text">Gender</th>
                  </tr>
                </thead>
                <tbody>
                <tr class="table-row">
